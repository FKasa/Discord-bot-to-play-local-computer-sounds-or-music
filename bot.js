...
function playAudio() {
  ...
  voiceChannel.join().then(connection => {
    ...
    //   if (audio.endsWith('.opus')) {
    //     break;
    //   }
    // }
    audio = list.shift();
    if(typeof audio === "undefined"){
      let bye = 'List finished. Leaving voice channel.';
      let statusChannel = bot.channels.cache.get(config.statusChannel);
      if (!statusChannel) return console.error('The status channel does not exist! Skipping.');
      statusChannel.send(bye).catch(console.log);
      console.log(bye);
...
bot.on('ready', () => {
  ...
  let statusChannel = bot.channels.cache.get(config.statusChannel);
  if (!statusChannel) return console.error('The status channel does not exist! Skipping.');
  statusChannel.send(readyEmbed).catch(console.error);
...
bot.on('message', async msg => {
  ...
  let command = msg.content.split(' ')[0].toLocaleLowerCase();
  ...
  if (command == 'git') {
    msg.reply('This is the source code of this project.\nhttps://github.com/Alee14/DLMP3').catch(console.error);
  }

  if (command == 'playing') {
    msg.channel.send('Currently playing `' + audio + '`.').catch(console.error);
  }
  
  if (command == 'about') {
    msg.channel.send('The bot code was adapted by Flamur Kasa from the original bot by Andrew Lee (Alee#4277). Written in Discord.JS and licensed with GPL-3.0.')
    .catch(console.error)
  }
  ...
  if (command == 'join') {
    msg.reply('Joining voice channel.').catch(console.error);
    console.log('Connected to the voice channel.');
    playAudio();
  }

  if (command == 'resume') {
    msg.reply('Resuming music.').catch(console.error);
    dispatcher.resume();
  }

  if (command == 'pause') {
    msg.reply('Pausing music.').catch(console.error);
    dispatcher.pause();
  }

  if (command == 'skip') {
    msg.reply('Skipping `' + audio + '`...').catch(console.error);
    dispatcher.pause();
    dispatcher = null;
    if(list.length)
    playAudio();
  ...
  if (command == 'leave') {
    voiceChannel = bot.channels.cache.get(config.voiceChannel);
    if (!voiceChannel) return console.error('The voice channel does not exist!\n(Have you looked at your configuration?)');
    msg.reply('Leaving voice channel.').catch(console.error);
  ...
  if (command == 'stop') {
    await msg.reply('Powering off...').catch(console.error);
    fileData = "Now Playing: Nothing";
    ...
    let statusChannel = bot.channels.cache.get(config.statusChannel);
    if (!statusChannel) return console.error('The status channel does not exist! Skipping.');
    await statusChannel.send(statusEmbed).catch(console.error);
